 ...
 * cd Uncle-L-random-wellness-notes && npx tsx generate-all.ts
 */
...
const JIMENG_URL = "https://ark.cn-beijing.volces.com/api/v3";
...
function validateConfig(): boolean {
  ...
  if (!ARK_API_KEY) missing.push("ARK_API_KEY");
  if (!ARK_MODEL) missing.push("ARK_MODEL");
  
  if (missing.length > 0) {
    console.error("\n❌ 错误：缺少以下环境变量配置：");
    missing.forEach(key => console.error(`   - ${key}`));
    console.error("\n请设置环境变量后重试：");
    console.error("   export DEEPSEEK_API_KEY=\"your-deepseek-api-key\"");
    console.error("   export ARK_API_KEY=\"your-volcengine-api-key\"");
    console.error("   export ARK_MODEL=\"your-model-endpoint-id\"");
    console.error("\n获取方式请参考 SKILL.md 文档\n");
    return false;
  }
...
async function generateImage(prompt: string, outputPath: string) {
  const response = await fetch(`${JIMENG_URL}/images/generations`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
